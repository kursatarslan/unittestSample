...
namespace AcceptanceTests.Pages
{
    public abstract class Page
    {
        ...
        public virtual void NgNavigateAbsolute(string path)
        {
            ...
            JsDriver.ExecuteAsyncScript(
                $"var callback = arguments[arguments.length - 1];ngNavigate('/').then(function () {{ return ngNavigate('{path}'); }}).then(callback, callback);");
            Wait();
        }
        ...
        public void Wait()
        {
            Driver.WaitUntil(driver => driver.FindElement(By.TagName("app-root")).FindElements(By.XPath(".//*")).Any());
        }
    }
}