...
namespace AcceptanceTests.Tests
{
    public class SmokeTest : BaseAcceptanceTest
    {
        ...
        {
            (DriverManager.Driver as IJavaScriptExecutor).ExecuteAsyncScript(
                "var callback = arguments[arguments.length - 1];ngNavigate('/').then(function () { return ngNavigate('/'); }).then(callback, callback);");
