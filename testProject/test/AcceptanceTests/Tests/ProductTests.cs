...
namespace AcceptanceTests.Tests
{
    public class ProductCreateTest : BaseAcceptanceTest<ProductCreatePage>
    {
        ...
        public async Task CreateProduct()
        {
            ...
            Page.Submit();
            Thread.Sleep(5000); // todo: use a clever wait to wait for Angular
        }

        [Test]
        public void UrlIsCorrect()
        {
            Page.CurrentUrl.Should().Be("http://localhost:7200/product");
        }
        ...
        [Test]
        public async Task ThereIsOnlyOneProductAfterCreating()
        {
            (await repository.GetAllAsync()).Count().Should().Be(1);
        }
    }
}